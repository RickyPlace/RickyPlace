(function() {
    ...
    // Confirmar colocación de imagen (doble clic en el lienzo)
            ...
            try {
                ...
                for (let cy = 0; cy < CHUNKS_Y; cy++) {
                    for (let cx = 0; cx < CHUNKS_X; cx++) {
                        updates[`lienzo/chunks/${cy}_${cx}`] = new Array(CHUNK_SIZE * CHUNK_SIZE).fill(0).map(idxToChar).join('');
                    }
                }
                updates['lienzo/count'] = 0;
                updates['lienzo/activity'] = { x: 0, y: 0, c: 0, t: firebase.database.ServerValue.TIMESTAMP };
